...
// owner_table_section.css({'margin-bottom': '20px', 'text-align': 'center'});
...
// date_range_section.css({'margin-bottom': '20px', 'text-align': 'center'});

// var data_section = $('<div>').addClass('data-section').appendTo(page.body);
// data_section.css({'margin-bottom': '20px', 'display': 'flex', 'align-items': 'center', 'flex-wrap': 'wrap'});
...
// pagination_section.css({'display': 'flex', 'justify-content': 'center', 'margin-top': '20px'});
...
// prevButton.css({'order': '1', 'margin-right': 'auto'});
// nextButton.css({'order': '2'});
...
//             args: { doctype, fields, filters, limit_start: start, limit_page_length: limit, order_by: 'raw_material asc' },
...
//         }
//     });
//     console.log('Clients populated:', Array.from(uniqueClients));
// });
...
//     });
//     console.log('Query types populated:', Array.from(uniqueQueryTypes));
// });

// // Function to fetch and display data for the selected client and query type
// function fetchData(client, queryType, page, limit) {
//     console.log('Fetching data with filters - Client:', client, 'Query Type:', queryType);
...
//         args: {
//             doctype: 'Query',
//             fields: ['raw_material', 'supplier', 'manufacturer', 'query_types', 'workflow_state', 'creation','client_name','client_code'],
//             filters,
...
//                 });
//             });
//             updateOwnerTable(client);
//             updateDateRange(filters);
//         }
//     });
// }
// function updateOwnerTable(selectedClient) {
//     console.log('Updating owner table for client:', selectedClient);
...
//                     $('<h2>').text('Account: ' + clientData.client_name + ' (Code: ' + clientData.client_code + ')').appendTo(firstRowData);
//                 }
//             }
//         });
//     } else {
...
// // Function to update date range based on filtered data
...
// // Event listeners for dropdown changes and input filtering
...
// ############################
    ...
    // // Fetch all data with proper callback chaining
    // function fetchAllData(doctype, fields, filters, callback) {
    //     var start = 0;
    //     var limit = 500;
    //     var allData = [];
...
>>>>>>> 8dec8ca4567c0ad696abc85290516dcddb4a3098
        ...
        frappe.call({
            ...
            callback: function(response) {
                ...
                data.forEach(function(item) {
                    ...
                    $('<td>').text(item.raw_material || '').appendTo(row);
                    $('<td>').text(item.supplier || '').appendTo(row);
                    $('<td>').text(item.manufacturer || '').appendTo(row);
                    $('<td>').text(item.query_types || '').appendTo(row);
                    $('<td>').text(item.workflow_state || '').appendTo(row);
                });
            }
        });
    }
    ...
    // Fetch data on client selection
    clientNameDropdown.on('change', function() {
        currentPage = 1;
        fetchData(clientNameDropdown.val(), queryTypeDropdown.val(), currentPage, itemsPerPage);
    });

    // Fetch data on query type selection
    queryTypeDropdown.on('change', function() {
        currentPage = 1;
        fetchData(clientNameDropdown.val(), queryTypeDropdown.val(), currentPage, itemsPerPage);
    });

    // Fetch data on filter input change
    filterRow.find('input').on('input', function() {
        currentPage = 1;
        fetchData(clientNameDropdown.val(), queryTypeDropdown.val(), currentPage, itemsPerPage);
    });
